...
<div class="centerColumn" id="accountHistInfo">
  ...
  <?php
  if (!$order || !zen_is_logged_in()) {
    echo TEXT_RETURN_REQUEST_INTRO;
    ...
    <fieldset>
      ...
      <?php echo zen_draw_password_field('password', '', zen_set_field_length(TABLE_CUSTOMERS, 'customers_password') . ' id="login-password"', 'password'); ?>
    </fieldset>

    <div class="buttonRow forward"><?php echo zen_image_submit(BUTTON_IMAGE_LOGIN, BUTTON_LOGIN_ALT); ?></div>
    <div class="buttonRow back important"><a href="<?php echo zen_href_link(FILENAME_PASSWORD_FORGOTTEN, '', 'SSL'); ?>"><?php echo TEXT_PASSWORD_FORGOTTEN; ?></a></div>
    <?php echo '</form>'; ?>
    <br class="clearBoth">
  <?php } ?>
          ...
          <tr>
            ...
            <td class="accountProductDisplay">
                ...
                <ul id="orderAttribsList">
                    ...
                    <li><?php echo $order->products[$i]['attributes'][$j]['option'] . TEXT_OPTION_DIVIDER . nl2br($order->products[$i]['attributes'][$j]['value']); ?></li>
                  <?php } ?>
                </ul>
              <?php } ?>
            </td>
  ...
  if ($order || zen_is_logged_in()) {
    ...
    $getDate = ("SELECT date_added
                 FROM " . TABLE_ORDERS_STATUS_HISTORY . "
                 WHERE orders_id = " . (int)$orderID . "
                 AND orders_status_id = " . (int)$compareDate);
    ...
    if ($comparing->fields['date_added'] <= $currentDate && zen_not_null($comparing->fields['date_added'])) {
      ?>
      <div id="returnSuccess"><?php echo TEXT_RETURN_GRACE_PERIOD_EXPIRED; ?></div>
    <?php } else { ?>


      <!--  BOF Flexible Returns Authorization (RMA) -->
      <?php if (DEFINE_BUTTON_LINK == '0') { ?>
        <div class="rmaRequestButton">
          <h4><?php echo TEXT_ACCOUNT_INFO_RETURNS_BUTTON_HEADER; ?></h4>
          <a href="<?php echo zen_href_link(FILENAME_RETURNS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'order_id=' . $_POST['order_id'], 'SSL'); ?>"><?php echo zen_image_button(BUTTON_IMAGE_SUBMIT, BUTTON_SUBMIT_ALT); ?></a>
        </div>
      <?php } else { ?>
        <div class="rmaRequestButton">
          <h4><?php echo TEXT_ACCOUNT_INFO_RETURNS_TEXT_LINK_HEADER; ?></h4>
          <?php echo TEXT_DEFINE_BUTTON_LINK; ?><a href="<?php echo zen_href_link(FILENAME_RETURNS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'order_id=' . $_POST['order_id'], 'SSL'); ?>"><?php echo TEXT_DEFINE_BUTTON_LINK2; ?></a>
          <?php echo TEXT_DEFINE_BUTTON_LINK3; ?></div> <?php } ?>
      <br class="clearBoth">
      <!--  EOF Flexible Returns Authorization (RMA) -->
    <?php } ?>
  <?php } ?>
  <?php echo zen_draw_form('rma_request', zen_href_link(FILENAME_RETURNS_REQUEST, '', 'SSL'), 'post') . zen_draw_hidden_field('action', 'process'); ?>
  <fieldset>
    <legend><?php echo HEADING_TITLE_1; ?></legend>
    <?php
    if (isset($_POST['action']) && $_POST['action'] == "process") {
      if ($errorInvalidID) {
        echo ERROR_INVALID_ORDER;
      }
    ...
    <?php echo zen_draw_input_field('query_email_address', '', 'size="35" id="query_email_address"', 'email'); ?> 
    <br class="clearBoth">
    <label class="inputLabel"><?php echo ENTRY_ORDER_NUMBER; ?></label>
    <?php echo zen_draw_input_field('order_id', '', 'size="20" id="order_id"', 'number'); ?> 
