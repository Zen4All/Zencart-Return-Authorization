...
<!--  BOF Flexible Returns Authorization (RMA) -->
            ...
            <td class="smallText" align="center"><strong><?php echo 'RMA Number'; ?></strong></td>

          </tr>
<?php

    $orders_history = $db->Execute("select orders_status_id, date_added, customer_notified, comments, rma_number, action
...
// BOF Flexible Returns Authorization (RMA)
        ...
        echo '            <td class="smallText">'; if (zen_not_null($orders_history->fields['action'])) { echo '<b>Action Requested: </b>' . $orders_history->fields['action'] . '<hr />'; } echo nl2br(zen_db_output($orders_history->fields['comments'])) . '&nbsp;</td>' . "\n";
        echo '            <td class="smallText">' . $orders_history->fields['rma_number'] . '&nbsp;</td>' . "\n" .
...
// indicate if comments exist
      $orders_history_query = $db->Execute("select orders_status_id, date_added, customer_notified, comments, rma_number, action from " . TABLE_ORDERS_STATUS_HISTORY . " where orders_id = '" . $oInfo->orders_id . "' and comments !='" . "'" );
